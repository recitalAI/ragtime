...
class ValidationSet(MethodView):
    ...
    def delete(self, name):
        ...
        try:
            search_pattern = os.path.join(VALIDATION_SETS_FOLDER, f"{name}_Validation_set_Q*_F*.json")
...
class ValidationSetContent(MethodView):
    ...
    def get(self, name):
        ...
        """
        try:
            pattern = os.path.join(VALIDATION_SETS_FOLDER, f"{name}_Validation_set_Q*_F*.json")
