"""Legacy file-based live-log capture, kept for the /api/live-logs endpoint
and the synchronous /api/start-experiment path. Job-based experiments
(POST /api/jobs) stream their logs through app.infra.job_store instead —
per-job buffers, no whole-file re-reads, no "silent for 30 s = done"
heuristic.
"""
...
# log_capture.py lives at app/infra/ — backups go to <backend>/logs.
...
class LogCapture:
    def __init__(self, log_file_path='/usr/local/bin/logs/logs.txt'):
        ...
        self.backup_path = os.path.join(_BACKEND_DIR, 'logs', 'experiment_logs.txt')
