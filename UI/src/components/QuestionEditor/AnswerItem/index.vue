  ...
  <v-card class="answer-item mb-4 golden-answer">
    <v-card-text>
      ...
      <div class="answer-meta text-caption">
        ...
        <p v-if="answer.llm_answer?.name !== 'Human' && typeof answer.llm_answer?.cost === 'number'" class="font-weight-medium">
          Cost: ${{ formatNumber(answer.llm_answer.cost) }}
        </p>
        <p v-if="typeof answer.llm_answer?.duration === 'number'" class="font-weight-medium">
...
export default {
  ...
  computed: {
    generatedBy() {
      const name = this.answer.llm_answer?.name || 'Unknown';
